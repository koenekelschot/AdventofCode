    ...
    class Day04
    {
        public static void Part1()
        {
            ...
            using (StringReader reader = new StringReader(Input))
            {
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    Debug.WriteLine($"Words: {words.Length}; Distinct: {distinct.Count()}");
            ...
            Debug.WriteLine($"Valid count: {numValid}");
            Debug.WriteLine($"Invalid count: {numInvalid}");
        }
        ...
        public static void Part2()
        {
            ...
            using (StringReader reader = new StringReader(Input))
            {
                ...
                {
                    var words = line.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                    words = words.Select(w => string.Join("", w.ToCharArray().OrderBy(c => c))).ToArray();
                    var distinct = words.Distinct();
                    Debug.WriteLine($"Words: {words.Length}; Distinct: {distinct.Count()}");
                    if (words.Length == distinct.Count())
                    {
