    ...
    class Day08
    {
        public static void Part1()
        {
            ...
            using (StringReader reader = new StringReader(Input))
            {
                ...
                {
                    //register [inc|dec] amount [if] register operator amount
                    string[] parts = line.Split(' ');
                    if (!registers.Keys.Contains(parts[0]))
                    {
                        registers.Add(parts[0], 0);
                    }
                    if (!registers.Keys.Contains(parts[4]))
                    {
                        registers.Add(parts[4], 0);
                    }
                    if (Evaluate(registers[parts[4]], parts[5], int.Parse(parts[6])))
                    {
            ...
            Debug.WriteLine("Max value in registers: " + maxRegisterValue);
        }
        ...
        public static void Part2()
        {
            ...
            Debug.WriteLine("Highest value evah in registers: " + highestValue);
        }

        private static bool Evaluate(int value, string @operator, int targetValue) {
            switch(@operator)
            {
...
//        private static string Input = @"b inc 5 if a > 1
//a inc 1 if b < 5
//c dec -10 if a >= 1
//c inc -20 if c == 10";
