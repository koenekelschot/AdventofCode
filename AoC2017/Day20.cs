...
namespace AoC2017
{
    class Day20
    {
        ...
        {
            //the GPU contacts you, asking for help. Someone has asked it to simulate too many particles, and it won't be able 
            //to finish them all in time to render the next frame at this rate. It transmits to you a buffer(your puzzle input) 
            //listing each particle in order (starting with particle 0, then particle 1, particle 2, and so on). For each 
            //particle, it provides the X, Y, and Z coordinates for the particle's position (p), velocity (v), and acceleration 
            //(a), each in the format <X,Y,Z>.
            ...
            using (StringReader reader = new StringReader(Input))
            {
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    {
                        var values = parts[i].Trim(new[] { 'p', 'v', 'a', '=', '<', '>' }).Split(new[] { ',' });
                        if (i == 0)
                        {
                            p.Position = new Tuple<long, long, long>(long.Parse(values[0]), long.Parse(values[1]), long.Parse(values[2]));
                        }
                        else if (i == 1)
                        {
                            p.Velocity = new Tuple<long, long, long>(long.Parse(values[0]), long.Parse(values[1]), long.Parse(values[2]));
                        }
                        else
                        {
                            p.Acceleration = new Tuple<long, long, long>(long.Parse(values[0]), long.Parse(values[1]), long.Parse(values[2]));
                        }
                    }
                    particles.Add(p);
                }
                ...
                for (var i = 0; i < 10000; i++)
                {
                    ...
                    {
                        var vX = p.Velocity.Item1 + p.Acceleration.Item1;
                        var vY = p.Velocity.Item2 + p.Acceleration.Item2;
                        var vZ = p.Velocity.Item3 + p.Acceleration.Item3;
                        p.Velocity = new Tuple<long, long, long>(vX, vY, vZ);

                        var pX = p.Position.Item1 + p.Velocity.Item1;
                        var pY = p.Position.Item2 + p.Velocity.Item2;
                        var pZ = p.Position.Item3 + p.Velocity.Item3;
                        p.Position = new Tuple<long, long, long>(pX, pY, pZ);
                    }

                    var minDistance = particles.Min(p => p.Distance);
                    Debug.WriteLine("Min distance: " + minDistance);
                    var particle = particles.First(p => p.Distance == minDistance);
                    Debug.WriteLine("Index: " + particles.IndexOf(particle));
                }
            }
        }
        ...
        public static void Part2()
        {
            ...
            using (StringReader reader = new StringReader(Input))
            {
                ...
                //bool destroyedParticles = true;

                //while (destroyedParticles)
                for (var i = 0; i < 100; i++)
                {
                    Debug.WriteLine($"[{i}]: {particles.Where(particle => !particle.IsDestroyed).Count()}");
                    foreach (var p in particles.Where(particle => !particle.IsDestroyed))
                    {
                        var vX = p.Velocity.Item1 + p.Acceleration.Item1;
                        var vY = p.Velocity.Item2 + p.Acceleration.Item2;
                        var vZ = p.Velocity.Item3 + p.Acceleration.Item3;
                    ...
                    foreach (var particle in particles.Where(particle => !particle.IsDestroyed))
                    {
                        var toDestroy = particles.Where(p => !p.IsDestroyed && p != particle &&
                        ...
                        //else
                        //{
                        //    destroyedParticles = false;
                        //}
                    }
                }

                Debug.WriteLine("Remaining particles: " + particles.Count(p => !p.IsDestroyed));
            }
        ...
        private class Particle
        {
            ...
            public Tuple<long, long, long> Velocity { get; set; }
            public Tuple<long, long, long> Acceleration { get; set; }
            public bool IsDestroyed = false;
            public long Distance => Math.Abs(Position.Item1) + Math.Abs(Position.Item2) + Math.Abs(Position.Item3);
        }

        private static string TestInput = @"p=<3,0,0>, v=<2,0,0>, a=<-1,0,0>
p=<4,0,0>, v=<0,0,0>, a=<-2,0,0>";

        private static string Input = @"p=<-1724,-1700,5620>, v=<44,-10,-107>, a=<2,6,-9>
