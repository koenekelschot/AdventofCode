...
namespace AoC2017
{
    class Day07
    {
        public static void Part1()
        {
            ...
            //immediately above them balancing on that disc. You write this information down (your puzzle input). Unfortunately, 
            //in their panic, they don't do this in an orderly fashion; by the time you're done, you're not sure which program gave 
            //which information.
            ...
            IDictionary<string, string[]> dictionary = new Dictionary<string, string[]>(); //child, parents[]
            string line;
            using (StringReader reader = new StringReader(Input))
            {
                while ((line = reader.ReadLine()) != null)
                {
                    var parentChilds = line.Split(new string[] { " -> " }, StringSplitOptions.RemoveEmptyEntries);
                    var parent = parentChilds[0].Split(' ')[0]; //weight = [1]
                    if (parentChilds.Length > 1)
                    {
                        var childs = parentChilds[1].Split(new string[] { ", " }, StringSplitOptions.RemoveEmptyEntries);
                        dictionary.Add(parent, childs.ToArray());
                    }
                }
            }

            var dictKeys = dictionary.Keys.ToArray();
            while (dictKeys.Length > 1)
            {
                Debug.WriteLine("Dictionary size: " + dictKeys.Length);
                foreach (string parent in dictKeys)
                {
                    if (!dictionary[parent].Any(child => dictionary.Keys.Contains(child)))
                    {
                        dictionary.Remove(parent);
                        Debug.WriteLine("Removed " + parent);
                    }
                }
                dictKeys = dictionary.Keys.ToArray();
            }

            Debug.WriteLine("Bottom program: " + dictKeys[0]);
        }

        public static void Part2()
        {
            IDictionary<string, string[]> dictionary = new Dictionary<string, string[]>(); //child, parents[]
            IDictionary<string, string[]> clonedDictionary = new Dictionary<string, string[]>(dictionary);
            IDictionary<string, int> weights = new Dictionary<string, int>();
            string line;
            using (StringReader reader = new StringReader(Input))
            {
                while ((line = reader.ReadLine()) != null)
                {
                    var parentChilds = line.Split(new string[] { " -> " }, StringSplitOptions.RemoveEmptyEntries);
                    var parent = parentChilds[0].Split(' ')[0]; //weight = [1]
                    var weight = parentChilds[0].Split(' ')[1].Trim('(', ')');
                    if (parentChilds.Length > 1)
                    {
                        var childs = parentChilds[1].Split(new string[] { ", " }, StringSplitOptions.RemoveEmptyEntries);
                        dictionary.Add(parent, childs.ToArray());
                        clonedDictionary.Add(parent, childs.ToArray());
                    }
                    else
                    {
                        //dictionary.Add(parent, new string[] { });
                        clonedDictionary.Add(parent, new string[] { });
                    }
                    weights.Add(parent, int.Parse(weight));
                }
            }
            ...
            //we hebben nu de root
            Node root = new Node
            {
                Name = dictKeys[0],
                Weight = weights[dictKeys[0]],
                Parent = null
            };
            root.SetChildren(clonedDictionary, weights);
            root.RestoreBalance();
        }
        ...
        private class Node
        {
            ...
            public bool IsTreeBalanced => Children.Select(c => c.TotalWeight).Distinct().Count() == 1;
            public bool IsCausingUnbalance => IsTreeBalanced && !Parent.IsTreeBalanced;

            public void SetChildren(IDictionary<string, string[]> dict, IDictionary<string, int> weights)
            {
                foreach (string child in dict[Name])
                {
                    var node = new Node
                    {
                        Name = child,
            ...
            {
                if (IsCausingUnbalance)
                {
                    var siblingWeights = Parent.Children.Where(c => c != this).Select(c => c.TotalWeight);
                    if (!siblingWeights.Any(sw => sw == TotalWeight))
                    {
                        var targetWeight = siblingWeights.First();
                        var diff = Math.Abs(targetWeight - TotalWeight);
                        Debug.WriteLine("Weight difference");
                        if (TotalWeight > targetWeight)
                        {
                            Debug.WriteLine("Decrease weight of node " + Name);
                            Debug.WriteLine(Weight - diff);
                        }
                        else
                        {
                            Debug.WriteLine("Increase weight of node " + Name);
                            Debug.WriteLine(Weight + diff);
                        }
                    }
                }
...
/*
        private static string Input = @"pbga (66)
xhth (57)
ebii (61)
havc (66)
ktlj (57)
...
cntj (57)";*/
        ...
        private static string Input = @"occxa (60)
