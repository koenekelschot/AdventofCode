...
namespace AoC2017
{
    class Day17
    {
        public static void Part1()
        {
            ...
            // -  0 (1): the spinlock steps forward three times (0, 0, 0), and then inserts the first value, 1, after 
            //   it. 1 becomes the current position.
            ...
            // -  0  5  7  2  4  3 (8) 6  1
            // -  0 (9) 5  7  2  4  3  8  6  1
            // Eventually, after 2017 insertions, the section of the circular buffer near the last insertion looks like 
            // this:
            ...
            {
                var stepsToTake = steps % buffer.Count;
                int newPosition = currentPosition + stepsToTake;
                if (newPosition >= buffer.Count)
                {
                    newPosition -= buffer.Count;
                }
                currentPosition = newPosition + 1;
                var bufferStart = buffer.Take(currentPosition);
                var bufferEnd = buffer.Skip(currentPosition);
                buffer = bufferStart.Union(new int[1] { i }).Union(bufferEnd).ToList();
            }
            Debug.WriteLine($"Last insert at pos {currentPosition}");
            Debug.WriteLine($"Value at {currentPosition + 1}: {buffer[currentPosition + 1]}");
        }
        ...
        public static void Part2()
        {
            ...
            for (var i = 1; i <= 50000000; i++)
            {
                ...
                {
                    Debug.WriteLine($"Insert {i} at pos {currentPosition}");
                }
            }
        }

        private static int TestInput = 3;
        private static int Input = 312;
    }
}
